 ...
 */
public class HadoopRelativeTableOperations implements TableOperations {
    private static final Logger LOG = LoggerFactory.getLogger(HadoopRelativeTableOperations.class);
    ...
    protected HadoopRelativeTableOperations(String warehouseLocation, Path location, FileIO fileIO, FileSystem fs, LockManager lockManager) {
        this.fs = fs;
    ...
    private static TableMetadata checkUUID(TableMetadata currentMetadata, TableMetadata newMetadata) {
        ...
        if (currentMetadata != null && currentMetadata.uuid() != null && newUUID != null) {
            Preconditions.checkState(newUUID.equals(currentMetadata.uuid()), "Table UUID does not match: current=%s != refreshed=%s", currentMetadata.uuid(), newUUID);
        }
        return newMetadata;
    }

    private Path absoluteLocation(Path location) {
        return new Path(warehouseLocation, location);
    }
    ...
    private synchronized void updateVersionAndMetadata(int newVersion, String metadataFile) {
        ...
        if (version == null || version != newVersion) {
            ...
            this.currentMetadata = checkUUID(currentMetadata, TableMetadataParser.read(io(), relativeLocation(metadataFile)));
        }
    }
    ...
    public void commit(TableMetadata base, TableMetadata metadata) {
        ...
        Preconditions.checkArgument(!metadata.properties().containsKey(TableProperties.WRITE_METADATA_LOCATION),
        ...
        renameToFinal(absoluteLocation(tempMetadataFile), absoluteLocation(finalMetadataFile), nextVersion);
    ...
    public TableOperations temp(TableMetadata uncommittedMetadata) {
        return new TableOperations() {
            ...
            @Override
            public String metadataFileLocation(String fileName) {
                return HadoopRelativeTableOperations.this.metadataFileLocation(fileName);
            }
            ...
            @Override
            public FileIO io() {
                return HadoopRelativeTableOperations.this.io();
            }

            @Override
            public EncryptionManager encryption() {
                return HadoopRelativeTableOperations.this.encryption();
            }

            @Override
            public long newSnapshotId() {
                return HadoopRelativeTableOperations.this.newSnapshotId();
            }
        };
    }
    ...
    Path getMetadataFile(int metadataVersion) throws IOException {
        for (TableMetadataParser.Codec codec : TABLE_METADATA_PARSER_CODEC_VALUES) {
            Path metadataFile = absoluteLocation(metadataFilePath(metadataVersion, codec));
    ...
    private void writeVersionHint(int versionToWrite) {
        Path versionHintFile = absoluteLocation(versionHintFile());

        try {
            Path tempVersionHintFile = absoluteLocation(metadataPath(UUID.randomUUID() + "-version-hint.temp"));
    ...
    int findVersion() {
        ...
        try (InputStreamReader fsr = new InputStreamReader(fs.open(versionHintFile), StandardCharsets.UTF_8); BufferedReader in = new BufferedReader(fsr)) {
            return Integer.parseInt(in.readLine().replace("\n", ""));
        } catch (Exception e) {
            try {
                Path root = absoluteLocation(metadataRoot());
                if (fs.exists(root)) {
                ...
                // available version
                FileStatus[] files = fs.listStatus(root, name -> VERSION_PATTERN.matcher(name.getName())
                                                                                          .matches());
                int maxVersion = 0;
     ...
     */
    private void renameToFinal(Path src, Path dst, int nextVersion) {
        try {
    ...
    private void deleteRemovedMetadataFiles(TableMetadata base, TableMetadata metadata) {
        ...
        boolean deleteAfterCommit = metadata.propertyAsBoolean(
                TableProperties.METADATA_DELETE_AFTER_COMMIT_ENABLED, TableProperties.METADATA_DELETE_AFTER_COMMIT_ENABLED_DEFAULT);
        ...
        if (deleteAfterCommit) {
            ...
            Tasks.foreach(removedPreviousMetadataFiles).executeWith(ThreadPools.getWorkerPool()).noRetry()
                 .suppressFailureWhenFinished()
                 .onFailure((previousMetadataFile, exc) -> LOG.warn("Delete failed for previous metadata file: {}", previousMetadataFile, exc))
