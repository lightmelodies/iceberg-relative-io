...
public class JdbcRelativeCatalog extends BaseMetastoreViewCatalog implements Configurable<Object>, SupportsNamespaces {
    static final String VIEW_WARNING_LOG_MESSAGE = "JDBC relative catalog is initialized without view support. To auto-migrate the database's schema and enable view support, set jdbc.schema-version=V1";
    private static final String NAMESPACE_EXISTS_PROPERTY = "exists";
    private static final Logger LOG = LoggerFactory.getLogger(JdbcRelativeCatalog.class);
    ...
    private FileIO io;
    private String catalogName = "jdbc-relative";
    private Object conf;
    ...
    public JdbcRelativeCatalog() {
        this(null, null, true);
    }

    public JdbcRelativeCatalog(Function<Map<String, String>, FileIO> ioBuilder, Function<Map<String, String>, JdbcClientPool> clientPoolBuilder, boolean initializeCatalogTables) {
    ...
    public void initialize(String name, Map<String, String> properties) {
        ...
        Preconditions.checkArgument(!Strings.isNullOrEmpty(inputWarehouseLocation), "Cannot initialize JdbcRelativeCatalog because warehousePath must not be null or empty");
        ...
        if (null != ioBuilder) {
            ...
            String ioImpl = properties.getOrDefault(CatalogProperties.FILE_IO_IMPL, RelativeFileIO.class.getName());
        ...
        this.initializeCatalogTables = PropertyUtil.propertyAsBoolean(properties, JdbcUtil.INIT_CATALOG_TABLES_PROPERTY, initializeCatalogTables);
    ...
    private void initializeCatalogTables() {
        ...
        try {
            connections.run(conn -> {
                ...
                ResultSet tableExists = dbMeta.getTables(null /* catalog name */, null /* schemaPattern */, JdbcUtil.CATALOG_TABLE_VIEW_NAME /* tableNamePattern */, null /* types */);
                if (tableExists.next()) {
                    return true;
                }

                LOG.debug("Creating table {} to store iceberg catalog tables", JdbcUtil.CATALOG_TABLE_VIEW_NAME);
                return conn.prepareStatement(JdbcUtil.V0_CREATE_CATALOG_SQL).execute();
            });

            connections.run(conn -> {
                DatabaseMetaData dbMeta = conn.getMetaData();
                ResultSet tableExists = dbMeta.getTables(null /* catalog name */, null /* schemaPattern */, JdbcUtil.NAMESPACE_PROPERTIES_TABLE_NAME /* tableNamePattern */, null /* types */);

                if (tableExists.next()) {
                    return true;
                }

                LOG.debug("Creating table {} to store iceberg catalog namespace properties", JdbcUtil.NAMESPACE_PROPERTIES_TABLE_NAME);
    ...
    private void updateSchemaIfRequired() {
        try {
            connections.run(conn -> {
                DatabaseMetaData dbMeta = conn.getMetaData();
                ResultSet typeColumn = dbMeta.getColumns(null, null, JdbcUtil.CATALOG_TABLE_VIEW_NAME, JdbcUtil.RECORD_TYPE);
                if (typeColumn.next()) {
                    LOG.debug("{} already supports views", JdbcUtil.CATALOG_TABLE_VIEW_NAME);
                    schemaVersion = JdbcUtil.SchemaVersion.V1;
                    return true;
                } else {
                    if (PropertyUtil.propertyAsString(catalogProperties, JdbcUtil.SCHEMA_VERSION_PROPERTY, JdbcUtil.SchemaVersion.V0.name())
    ...
    protected TableOperations newTableOps(TableIdentifier tableIdentifier) {
        return new JdbcTableOperations(connections, io, catalogName, tableIdentifier, catalogProperties, schemaVersion);
    }
    ...
    public boolean dropTable(TableIdentifier identifier, boolean purge) {
        ...
        int deletedRecords = execute((schemaVersion == JdbcUtil.SchemaVersion.V1) ? JdbcUtil.V1_DROP_TABLE_SQL :
                JdbcUtil.V0_DROP_TABLE_SQL, catalogName, JdbcUtil.namespaceToString(identifier.namespace()), identifier.name());
    ...
    public List<TableIdentifier> listTables(Namespace namespace) {
        ...
        return fetch(row -> JdbcUtil.stringToTableIdentifier(row.getString(JdbcUtil.TABLE_NAMESPACE), row.getString(JdbcUtil.TABLE_NAME)), (schemaVersion == JdbcUtil.SchemaVersion.V1) ? JdbcUtil.V1_LIST_TABLE_SQL : JdbcUtil.V0_LIST_TABLE_SQL, catalogName, JdbcUtil.namespaceToString(namespace));
    }
    ...
    public void renameTable(TableIdentifier from, TableIdentifier to) {
        ...
        int updatedRecords = execute(err -> {
                    // SQLite doesn't set SQLState or throw SQLIntegrityConstraintViolationException
                    if (err instanceof SQLIntegrityConstraintViolationException || (err.getMessage() != null && err.getMessage()
                                                                                                                   .contains("constraint failed"))) {
                        throw new AlreadyExistsException("Table already exists: %s", to);
                    }
                }, (schemaVersion == JdbcUtil.SchemaVersion.V1) ? JdbcUtil.V1_RENAME_TABLE_SQL : JdbcUtil.V0_RENAME_TABLE_SQL,
                JdbcUtil.namespaceToString(to.namespace()), to.name(), catalogName, JdbcUtil.namespaceToString(from.namespace()), from.name());
    ...
    public void createNamespace(Namespace namespace, Map<String, String> metadata) {
        ...
        if (metadata == null || metadata.isEmpty()) {
            ...
            createMetadata = ImmutableMap.<String, String>builder().putAll(metadata)
                                         .put(NAMESPACE_EXISTS_PROPERTY, "true").buildOrThrow();
        }
    ...
    public List<Namespace> listNamespaces() {
        ...
        namespaces.addAll(fetch(row -> JdbcUtil.stringToNamespace(row.getString(JdbcUtil.TABLE_NAMESPACE)), JdbcUtil.LIST_ALL_NAMESPACES_SQL, catalogName));
        namespaces.addAll(fetch(row -> JdbcUtil.stringToNamespace(row.getString(JdbcUtil.NAMESPACE_NAME)), JdbcUtil.LIST_ALL_PROPERTY_NAMESPACES_SQL, catalogName));
    ...
    public List<Namespace> listNamespaces(Namespace namespace) throws NoSuchNamespaceException {
        ...
        List<Namespace> namespaces = Lists.newArrayList();
        namespaces.addAll(fetch(row -> JdbcUtil.stringToNamespace(row.getString(JdbcUtil.TABLE_NAMESPACE)), JdbcUtil.LIST_NAMESPACES_SQL, catalogName, JdbcUtil.namespaceToString(namespace) + "%"));
        namespaces.addAll(fetch(row -> JdbcUtil.stringToNamespace(row.getString(JdbcUtil.NAMESPACE_NAME)), JdbcUtil.LIST_PROPERTY_NAMESPACES_SQL, catalogName, JdbcUtil.namespaceToString(namespace) + "%"));
        ...
        namespaces = namespaces.stream()
                               ...
                               // only get sub namespaces/children
                               .map(n -> Namespace.of(Arrays.stream(n.levels()).limit(subNamespaceLevelLength)
                                                            .toArray(String[]::new)))
                               // remove duplicates
                               .distinct()
    ...
    private String defaultNamespaceLocation(Namespace namespace) {
        return namespace.isEmpty() ? "" : SLASH.join(namespace.levels());
    }
    ...
    public boolean dropNamespace(Namespace namespace) throws NamespaceNotEmptyException {
        ...
        int deletedRows = execute(JdbcUtil.DELETE_ALL_NAMESPACE_PROPERTIES_SQL, catalogName, JdbcUtil.namespaceToString(namespace));
    ...
    public boolean setProperties(Namespace namespace, Map<String, String> properties) throws NoSuchNamespaceException {
        ...
        Preconditions.checkArgument(!properties.containsKey(NAMESPACE_EXISTS_PROPERTY), "Cannot set reserved property: %s", NAMESPACE_EXISTS_PROPERTY);
    ...
    public boolean dropView(TableIdentifier identifier) {
        ...
        int deletedRecords = execute(JdbcUtil.DROP_VIEW_SQL, catalogName, JdbcUtil.namespaceToString(identifier.namespace()), identifier.name());
    ...
    public List<TableIdentifier> listViews(Namespace namespace) {
        ...
        return fetch(row -> JdbcUtil.stringToTableIdentifier(row.getString(JdbcUtil.TABLE_NAMESPACE),
                row.getString(JdbcUtil.TABLE_NAME)), JdbcUtil.LIST_VIEW_SQL, catalogName, JdbcUtil.namespaceToString(namespace));
    }
    ...
    public void renameView(TableIdentifier from, TableIdentifier to) {
        ...
        }, JdbcUtil.RENAME_VIEW_SQL, JdbcUtil.namespaceToString(to.namespace()), to.name(), catalogName, JdbcUtil.namespaceToString(from.namespace()), from.name());
        ...
        if (updatedRecords == 1) {
            ...
            LOG.warn("Rename operation affected {} rows: the catalog view's primary key assumption has been violated", updatedRecords);
        }
    }
    ...
    private Map<String, String> fetchProperties(Namespace namespace) {
        ...
        List<Map.Entry<String, String>> entries = fetch(row -> new AbstractMap.SimpleImmutableEntry<>(
                        row.getString(JdbcUtil.NAMESPACE_PROPERTY_KEY), row.getString(JdbcUtil.NAMESPACE_PROPERTY_VALUE)),
